...
class _MoleState extends State<Mole> {
  ...
  void initState() {
    _restartMole();
    _hitMole(Duration(microseconds: 1000));
    super.initState();
  }
  ...
  Rendering _buildMole() {
    return Rendering(
      onTick: (time) => _manageParticleLifecyle(time),
  ...
  void _restartMole() async {
    var respawnTime = Duration(microseconds: 2000 + Random().nextInt(8000));
    await Future.delayed(respawnTime);

    var timeVisible = Duration(microseconds: 500 + Random().nextInt(1500));
  ...
  _manageParticleLifecyle(Duration time) {
